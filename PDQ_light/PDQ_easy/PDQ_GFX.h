...
{
	uint8_t i;
	for(i=0; i<sizeof(selchr); i++)
		if ((char)pgm_read_byte(&selchr[i]) == c)	// pgm_read_byte(selchr[i]) <> pgm_read_byte(selchr + i) !! De ce?! LMA
			break;									// pt ca: pgm_read_byte(uint8_c *adresa), iar selchr[i] este char, nu char* !
	if(i>=sizeof(selchr))
		return 0;
	GFXglyph *glyph = &(((GFXglyph *)pgm_read_pointer(&gfxFont->glyph))[i]);
	HW::drawCharGFX(cursor_x, cursor_y, i, textcolor, textbgcolor, textsize);
    ...
    if (size == 1)
    {
      ...
      for (coord_t yy=0; yy<h; yy++, _y++)
      {
        ...
        for (coord_t xx=0; xx<w; xx++, _x++)
        {
		  ...
		  else	 // Adaugat de mine, sa suprascrie fostul caracter. LMA
		  {
			HW::drawPixel(_x, _y, bg);
		  }
          bits <<= 1;
        }
      }
